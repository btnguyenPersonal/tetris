...
class GameGrid:
  ...
  def setBorderBottomLine(self, i):
    if i >= 0:
      renderGrid = copy.deepcopy(self.grid)
      try:
        for j in range(len(renderGrid[i])):
          if ((renderGrid[i][j] % 100) >> 1) % 2 == 1 and int(renderGrid[i][j] / 100) == 99:
            self.grid[i][j] -= 2
      except IndexError:
        return False
    return True

  def setBorderTopLine(self, i):
    if i >= 0:
      renderGrid = copy.deepcopy(self.grid)
      try:
        for j in range(len(renderGrid[i])):
          if (renderGrid[i][j] % 100) % 2 == 1 and int(renderGrid[i][j] / 100) == 99:
            self.grid[i][j] -= 1
      except IndexError:
        return False
    return True
